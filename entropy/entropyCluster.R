...
#BEGIN SIMULATION
ruggednessMatrix <- matrix(rep(0,120), ncol=15) #final output; ncol = 15 environments, nrow = 8 epsilon values
...
for (env_num in 1:15){
	...
	#find max entropy, over all tested values of epsilon, and assign to ruggednessMatrix for environment
	ruggednessMatrix[,env_num] <- H_epsilonVec
}	
...
save(ruggednessMatrix, file=name)
